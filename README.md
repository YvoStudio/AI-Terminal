...
### AI 优化
- **笔记队列 · 自动提交 AI** — 侧边笔记区可堆放多个文本块(支持粘贴/拖入图片),开启自动发送后,每当 Claude 一轮回答结束、回到空闲态,队列顶部的文本块就会作为下一条 prompt 自动注入并回车提交,逐条排队推进,无需手动盯着切换;手动「发送」则只把内容放进输入框供你检查。图片以「真实粘贴」方式注入,生成可点击的 `[Image #N]` 引用
